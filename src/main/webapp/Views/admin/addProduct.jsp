		...
		<div class="account-top heading">
			...
			<div class="account-main">
				<div class="col-md-6 account-left">
                    ...
                    <form action="/admin/addProductProcess" method="post" modelAttribute="product">
                    ${message.get('name') }
			    ...
			    <input placeholder="price" type="text" tabindex="4" required name="price" value="${product.price }">
					...
					<select name="colores" multiple="multiple">
		              <c:forEach var="color" items="${Colors }">  
							<option ${product.colors.name.equals(color.name) ? 'selected' : '' } value="${color.id }" >${color.name}</option>
								</c:forEach>
					 </select></td></tr>
					<tr>
					<td>${message.get('size') }</td>
		              ...
		              <c:forEach var="size" items="${Sizes }">  
							<option ${product.sizes.name.equals(size.name) ? 'selected' : '' } value="${size.id}">${size.name}</option>
					...
					<tr>
					 ...
					 <select name="offers" multiple="multiple">
		              <c:forEach var="offer" items="${Offers }">  
							<option ${product.offer.name.equals(offer.name) ? 'selected' : '' } value="${offer.id}">${offer.name}</option>
